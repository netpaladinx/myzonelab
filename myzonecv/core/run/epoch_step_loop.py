...
@RUNS.register_class('epoch_step_loop')
class EpochStepLoop(BaseRun):
    def __init__(self, name='epoch_step_loop', stage_names=None):
        super().__init__(name, stage_names)

    def __call__(self, ctx):
        self.call_stage('run_begin', ctx)

        for _ in self.call_iter_stage('iter_epochs', ctx):
            self.call_stage('epoch_begin', ctx)
