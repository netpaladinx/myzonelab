...
class Yolov5Detect6(BaseModule):
    ...
    def init_weights(self):
        ...
        for i in range(self.in_layers):
            normal_init(self.convs[i], std=0.02)
...
@BLOCKS.register_class('yolov5_detect6_myzone')
class Yolov5Detect6MyZone(Yolov5Detect6):
    """ classes (myzone: 9):
            fighter-left, fighter-right, fighter-near, fighter-far, fighter-up, fighter-down, 
            figher-crowd, fighter-single, fighter-none
        anchors: list(list), n_layers x (n_anchors*duplicates*2)
    """
    default_bbox_dims = 5
    ...
    def __init__(self,
        ...
        self.in_channels = in_channels
        self.n_classes = self.default_num_classes
        self.out_dims = self.n_classes + self.default_bbox_dims  # 14
        assert len(in_channels) == len(anchors)
        self.in_layers = len(in_channels)
        self.n_anchors = len(anchors[0]) // 2 * self.duplicates  # 6
        self.out_channels = self.out_dims * self.n_anchors  # 84

        self.convs = ModuleList(nn.Conv2d(c, self.out_channels, 1) for c in in_channels)


@BLOCKS.register_class('yolov5_detect6_myzone_v2')
class Yolov5Detect6MyZoneV2(Yolov5Detect6):
    """ classes (myzone: 9):
            fighter-left, fighter-right, fighter-near, fighter-far, fighter-up, fighter-down, 
            figher-crowd, fighter-single, fighter-none
        anchors: list(list), n_layers x (n_anchors*2)
    ...
    def __init__(self,
                 ...
                 anchors,
                 init_cfg=None):
        super().__init__(in_channels, self.default_num_classes, anchors, init_cfg)


@BLOCKS.register_class('yolov5_detect6_myzone_v3')
class Yolov5Detect6MyZoneV3(Yolov5Detect6):
    """ classes (myzone: 9):
            fighter-left, fighter-right, fighter-near, fighter-far, fighter-up, fighter-down, 
            figher-crowd, fighter-single, fighter-none
        anchors: list(list), n_layers x (n_anchors*2)
    ...
    def __init__(self,
        ...
        self.in_channels = in_channels
        self.n_classes = self.default_num_classes
        self.out_dims = self.default_bbox_dims * self.n_classes  # 5 * 9 = 45
        assert len(in_channels) == len(anchors)
        self.in_layers = len(in_channels)
        self.anchors = anchors
        self.n_anchors = len(anchors[0]) // 2
        self.out_channels = self.out_dims * self.n_anchors
    ...
    def init_weights(self):
        ...
        # initialize biases (https://arxiv.org/abs/1708.02002 section 3.3)
        for conv, stride in zip(self.convs, self.strides):
            bias = conv.bias.view(self.n_anchors * self.n_classes, -1)  # (n_anchors*n_classes) x bbox_dims
            bias.data[:, 4] += math.log(avg_objs / ((input_size[0] / stride) * (input_size[1] / stride)) / self.n_classes)  # objectness
            conv.bias = torch.nn.Parameter(bias.view(-1), requires_grad=True)


@BLOCKS.register_class('yolov5_detect6_myzone_v4')
class Yolov5Detect6MyZoneV4(Yolov5Detect6):
    """ classes (myzone: 9):
            fighter-left, fighter-right, fighter-near, fighter-far, fighter-up, fighter-down, 
            figher-crowd, fighter-single, fighter-none
        anchors: list(list), n_layers x (n_anchors*2)
    ...
    def __init__(self,
        ...
        self.in_channels = in_channels
        self.n_classes = self.default_num_classes
        self.out_dims = (self.default_bbox_dims + 1) * self.n_classes  # 6 * 9 = 54
        assert len(in_channels) == len(anchors)
        self.in_layers = len(in_channels)
        self.anchors = anchors
        self.n_anchors = len(anchors[0]) // 2
    ...
    def init_weights(self):
        ...
        # initialize biases (https://arxiv.org/abs/1708.02002 section 3.3)
        for conv, stride in zip(self.convs, self.strides):
            bias = conv.bias.view(self.n_anchors * self.n_classes, -1)  # (n_anchors*n_classes) x (bbox_dims+1)
            bias.data[:, 4] += math.log(avg_objs / ((input_size[0] / stride) * (input_size[1] / stride)) / self.n_classes)  # objectness
            bias.data[:, 5:] += math.log(prob_all_cls)  # classes
            conv.bias = torch.nn.Parameter(bias.view(-1), requires_grad=True)
...
@HEADS.register_class('yolov5_head6_myzone')
class Yolov5Head6MyZone(Yolov5Head6):
    detect_block_type = 'yolov5_detect6_myzone'


@HEADS.register_class('yolov5_head6_myzone_v2')
class Yolov5Head6MyZoneV2(Yolov5Head6):
    detect_block_type = 'yolov5_detect6_myzone_v2'


@HEADS.register_class('yolov5_head6_myzone_v3')
class Yolov5Head6MyZoneV3(Yolov5Head6):
    detect_block_type = 'yolov5_detect6_myzone_v3'


@HEADS.register_class('yolov5_head6_myzone_v4')
class Yolov5Head6MyZoneV4(Yolov5Head6):
    detect_block_type = 'yolov5_detect6_myzone_v4'
