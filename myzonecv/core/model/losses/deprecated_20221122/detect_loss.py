...
from ...data.dataconsts import CAT_ID_FIGHTER_NONE
...
@DETECT_LOSSES.register_class('bbox_obj_cls_myzone')
class DetectBBoxObjClsMyZone(BaseModule):
    def __init__(self, cls_pos_weight=1., obj_pos_weight=1., focal_loss_gamma=0., GIoU=False, DIoU=False, CIoU=False,
                 cls_loss_weight=1., obj_loss_weight=1., bbox_loss_weight=1., layer_obj_weights=(1., 1., 1.),
                 label_smoothing=0.1, eps=1e-10, cxy_margin=0.0, anchor_thr=2.9, wh_margin=0.05, wh_gamma=2):
        super().__init__()
        self.cls_bce = DetectClsBCE(cls_pos_weight, focal_loss_gamma, cls_loss_weight)
        self.obj_bce = DetectObjBCE(obj_pos_weight, focal_loss_gamma, obj_loss_weight)
        self.bbox_iou = DetectBBoxIoU('cxywh', GIoU, DIoU, CIoU, bbox_loss_weight, eps)
    ...
    def forward(self, output, target_cxywh, target_cij, target_cls, target_anc_idx, target_anc, target_cnt):
        ...
        for i, layer_out in enumerate(output):  # anchor layer i
            ...
            if n_tar > 0:
                ...
                # regression loss
                cxy = pred_out[:, :2].sigmoid() * (2 + self.cxy_margin * 2) - (0.5 + self.cxy_margin)  # -0.55 ~ 1.55
                # wh = (pred_out[:, 2:4].sigmoid() * 2) ** 2 * tar_anc  # 0 ~ 4 * anchor
                wh = (pred_out[:, 2:4].sigmoid()**self.wh_gamma) * (self.anchor_thr + self.wh_margin * 2) * tar_anc  # 0 ~ 3 * anchor (given anchor_thr=2.9)
                pred_bbox = torch.cat((cxy, wh), 1)  # n_tar x 4
                bbox_loss, iou_score = self.bbox_iou(pred_bbox, tar_cxywh)  # iou_score: n_tar, tar_cxywh: n_tar x 4
                loss_bbox += bbox_loss
                ...
                loss_cls += cls_loss
            # else:
            #     # assume cls is "fighter-none"
            #     pred_out = layer_out.reshape(-1, layer_out.shape[-1])  # n_all x out_dims
            #     pred_cls_logit = pred_out[:, 5:]
            #     tar_cls_prob = torch.full_like(pred_cls_logit, self.cls_neg_smoothing).to(device)
            #     tar_cls_prob[:, CAT_ID_FIGHTER_NONE] = self.cls_pos_smoothing
            #     cls_loss = self.cls_bce(pred_cls_logit, tar_cls_prob)  # tar_cls_prob: n_tar x n_cls
            #     loss_cls += cls_loss
...
@DETECT_LOSSES.register_class('bbox_obj_cls_myzone_v2')
class DetectBBoxObjClsMyZoneV2(BaseModule):
    def __init__(self, obj_pos_weight=1., focal_loss_gamma=0., GIoU=False, DIoU=False, CIoU=False,
                 obj_loss_weight=1., bbox_loss_weight=1., layer_obj_weights=(1., 1., 1.), obj_neg_smoothing=0.1, none_pos_smoothing=0.9,
                 eps=1e-10, cxy_margin=0.0, anchor_thr=2.9, wh_margin=0.05, wh_gamma=2):
        super().__init__()
        self.obj_bce = DetectObjBCE(obj_pos_weight, focal_loss_gamma, obj_loss_weight)
        self.bbox_iou = DetectBBoxIoU('cxywh', GIoU, DIoU, CIoU, bbox_loss_weight, eps)

        self.layer_obj_weights = layer_obj_weights
        self.obj_neg_smoothing = obj_neg_smoothing
        self.none_pos_smoothing = none_pos_smoothing
        self.eps = eps
        self.cxy_margin = cxy_margin
        self.anchor_thr = anchor_thr
        self.wh_margin = wh_margin
    ...
    def forward(self, output, target_cxywh, target_cij, target_cls, target_anc_idx, target_anc, target_cnt):
        """ output: list(bs x n_anchors x h x w x (n_classes*bbox_dims) (bbox_dims: cx,cy,w,h,obj)
            target_cxywh: list(n_tar x 4)
            target_cij: list(n_tar x 2)
            target_cls: list(n_tar)
            target_anc: list(n_tar x 2)
        ...
        for i, layer_out in enumerate(output):  # anchor layer i
            ...
            n_tar = tar_cnt.sum()
            bs = len(tar_cnt)
            bs_, n_anc, out_h, out_w, out_dims = layer_out.shape
            assert bs == bs_
            bbox_dims = 5
            n_cls = int(out_dims / bbox_dims)
            layer_out = layer_out.view(bs, n_anc, out_h, out_w, n_cls, bbox_dims)  # bs x n_anchors x h x w x n_classes x bbox_dims(5)
            tar_obj_prob = torch.full((bs, n_anc, out_h, out_w, n_cls), self.obj_neg_smoothing).to(device)  # bs x n_anchors x h x w x n_classes
            ...
            if n_tar > 0:
                ...
                # layer_out: bs x n_anchors x h x w x n_classes x bbox_dims(5) (!!!notice: h x w not w x h)
                pred_out = layer_out[batch_idx, tar_anc_idx, tar_cij[:, 1], tar_cij[:, 0], tar_cls]  # n_tar x bbox_dims(5)

                # regression loss
                cxy = pred_out[:, :2].sigmoid() * (2 + self.cxy_margin * 2) - (0.5 + self.cxy_margin)  # -0.5 - cxy_margin ~ 1.5 + cxy_margin
                # wh = (pred_out[:, 2:4].sigmoid() * 2) ** 2 * tar_anc  # 0 ~ 4 * anchor
                wh = (pred_out[:, 2:4].sigmoid()**self.wh_gamma) * (self.anchor_thr + self.wh_margin) * tar_anc  # 0 ~ (anchor_thr + wh_margin)
                pred_bbox = torch.cat((cxy, wh), 1)  # n_tar x 4
                bbox_loss, iou_score = self.bbox_iou(pred_bbox, tar_cxywh)  # iou_score: n_tar, tar_cxywh: n_tar x 4
                loss_bbox += bbox_loss

                # set objectness by iou
                score = iou_score.detach().clamp(self.obj_neg_smoothing)
                # !!!notice: h x w not w x h
                tar_obj_prob[batch_idx, tar_anc_idx, tar_cij[:, 1], tar_cij[:, 0], tar_cls] = score
            # else:
            #     # assume cls is "fighter-none"
            #     tar_obj_prob[..., CAT_ID_FIGHTER_NONE] = self.none_pos_smoothing

            # objectness loss
            pred_obj_logit = layer_out[..., 4]
            obj_loss = self.obj_bce(pred_obj_logit, tar_obj_prob)  # tar_obj_prob: bs x n_anchors x h x w x n_classes
            loss_obj += obj_loss * self.layer_obj_weights[i]

        loss = loss_bbox + loss_obj
        return {'loss': loss, 'lbbox': loss_bbox.detach(), 'lobj': loss_obj.detach()}


@DETECT_LOSSES.register_class('bbox_obj_cls_myzone_v3')
class DetectBBoxObjClsMyZoneV3(BaseModule):
    def __init__(self, cls_pos_weight=1., obj_pos_weight=1., focal_loss_gamma=0., GIoU=False, DIoU=False, CIoU=False,
                 cls_loss_weight=1., obj_loss_weight=1., bbox_loss_weight=1., layer_obj_weights=(1., 1., 1.),
                 label_smoothing=0.1, none_pos_smoothing=0.8, eps=1e-10, cxy_margin=0.01, anchor_thr=2.9, wh_margin=0.0, wh_gamma=2):
        super().__init__()
        self.cls_bce = DetectClsBCE(cls_pos_weight, focal_loss_gamma, cls_loss_weight)
        self.obj_bce = DetectObjBCE(obj_pos_weight, focal_loss_gamma, obj_loss_weight)
        self.bbox_iou = DetectBBoxIoU('cxywh', GIoU, DIoU, CIoU, bbox_loss_weight, eps)
    ...
    def forward(self, output, target_cxywh, target_cij, target_cls, target_anc_idx, target_anc, target_cnt):
        """ output: list(bs x n_anchors x h x w x (n_classes*(bbox_dims+1)) (bbox_dims: cx,cy,w,h,obj)
            target_cxywh: list(n_tar x 4)
            target_cij: list(n_tar x 2)
            target_cls: list(n_tar)
            target_anc: list(n_tar x 2)
        ...
        for i, layer_out in enumerate(output):  # anchor layer i
            ...
            assert bs == bs_
            bbox_dims = 5
            n_cls = int(out_dims / (bbox_dims + 1))
            layer_out = layer_out.view(bs, n_anc, out_h, out_w, n_cls, bbox_dims + 1)  # bs x n_anchors x h x w x n_classes x (bbox_dims+1)(6)
            tar_obj_prob = torch.zeros((bs, n_anc, out_h, out_w, n_cls)).to(device)  # bs x n_anchors x h x w x n_classes
            ...
            if n_tar > 0:
                ...
                # layer_out: bs x n_anchors x h x w x n_classes x (bbox_dims+1)(6) (!!!notice: h x w not w x h)
                pred_out = layer_out[batch_idx, tar_anc_idx, tar_cij[:, 1], tar_cij[:, 0]]  # n_tar x n_classes x (bbox_dims+1)(6)

                # regression loss
                cxy = pred_out[range(n_tar), tar_cls, :2].sigmoid() * (2 + self.cxy_margin * 2) - (0.5 + self.cxy_margin)  # -0.5 - cxy_margin ~ 1.5 + cxy_margin
                # wh = (pred_out[:, tar_cls, 2:4].sigmoid() * 2) ** 2 * tar_anc  # 0 ~ 4 * anchor
                wh = (pred_out[range(n_tar), tar_cls, 2:4].sigmoid()**self.wh_gamma) * (self.anchor_thr + self.wh_margin) * tar_anc  # 0 ~ (anchor_thr + wh_margin)
                pred_bbox = torch.cat((cxy, wh), 1)  # n_tar x 4
                bbox_loss, iou_score = self.bbox_iou(pred_bbox, tar_cxywh)  # iou_score: n_tar, tar_cxywh: n_tar x 4
                loss_bbox += bbox_loss
                ...
                # classification loss
                pred_cls_logit = pred_out[..., 5]  # n_tar x n_classes
                tar_cls_prob = torch.full_like(pred_cls_logit, self.cls_neg_smoothing).to(device)  # n_tar x n_classes
                tar_cls_prob[range(n_tar), tar_cls] = self.cls_pos_smoothing
                cls_loss = self.cls_bce(pred_cls_logit, tar_cls_prob)  # tar_cls_prob: n_tar x n_cls
                loss_cls += cls_loss
            # else:
            #     # assume cls is "fighter-none"
            #     pred_out = layer_out.reshape(-1, n_cls, bbox_dims + 1)  # n_all x n_classes x (bbox_dims+1)(6)
            #     pred_cls_logit = pred_out[:, CAT_ID_FIGHTER_NONE, 5]  # n_all
            #     tar_cls_prob = torch.full_like(pred_cls_logit, 1 - self.none_pos_smoothing).to(device)
            #     tar_cls_prob[:, CAT_ID_FIGHTER_NONE] = self.none_pos_smoothing
            #     cls_loss = self.cls_bce(pred_cls_logit, tar_cls_prob)  # tar_cls_prob: n_tar x n_cls
            #     loss_cls += cls_loss
