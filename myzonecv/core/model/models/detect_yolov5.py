...
class DetectYolov5(BaseModel):
    ...
    def forward_dummy(self, img, return_preds=True):
        ...
        output = self.head(output, backbone_outputs=outputs)
        if return_preds:
            output = torch.cat(self.predict(output), dim=1)[..., :6]  # bs x n_all_preds x 6 (cx, cy, w, h, obj_conf, cls_conf)
        return output
    ...
    def init_weights(self):
        ...
        init_cfg = self.init_cfg[0] if isinstance(self.init_cfg, (list, tuple)) else self.init_cfg
        if isinstance(init_cfg, dict) and init_cfg['type'] in ('pretrained', 'checkpoint'):
            return
